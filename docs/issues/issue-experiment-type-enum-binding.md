...
- `backend/services/bulk_uploads/experiment_status.py:17` (`_OCCUPANCY_TYPES = {"hpht", "core flood"}`), `21-23` (`_normalize_type`), `26-28` (`_is_eligible_for_occupancy`)

Note the inconsistency: `experiment_status.py` is the only site that normalizes case and whitespace before comparing. Its `_normalize_type` docstring explicitly anticipates `'HPHT '` and `'Core  Flood'` arriving. So one module already assumes dirty data is possible while every other module assumes it isn't. That disagreement is itself the smell.
...
- `_is_eligible_for_occupancy` *does* match it (it normalizes), so it participates in demotion while being invisible in the UI. That combination is the nastiest one: an experiment that can auto-complete other experiments but that nobody can see.
