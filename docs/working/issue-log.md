...
- **No schema changed.** `git diff --stat develop -- database/models/` is empty; `git diff --stat develop -- backend/services/calculations/ database/event_listeners.py` is empty. All work is parser logic in one locked file plus documentation.
- **Files changed:**
...
- **Pre-existing parser bug found, confirmed NOT fixed by this branch, recommended follow-up:** `str(row.get("Experiment ID") or "").strip()` in `_resolve_row_identity` does not treat a blank cell arriving as `float('nan')` as blank — NaN is truthy in Python, so the row's experiment ID becomes the literal string `"nan"` instead of being skipped. v2 has 21 such rows. Pre-existing effect: spurious "not found" errors (each `"nan"` row looks up an experiment that doesn't exist). New effect introduced by this branch's duplicate detection: multiple `"nan"` rows at the same Duration now also collide with each other and additionally emit duplicate-row errors on top of the not-found ones. Left unfixed — out of this plan's scope; recommend a follow-up issue to make `_resolve_row_identity` treat NaN the same as `None`/empty string.
