...
## 2026-09-23 | issue #118 — PR1–PR3 and the Titanium branch merged into develop (`/complete-task`)
- **Merged:** `feat/typed-notes-schema` (#119, `2ff6921`) → `chore/reclassify-notes-backfill` (#120, `8cfa368`) → `feat/typed-notes-readers` (#121, `15ab581`), each `--no-ff`. PR3 carried `feat/icp-add-titanium` (`480277d`) and the re-parented chain, so Ti arrived inside it with no conflicts and develop never held two Alembic heads. GitHub registered all three PRs as merged; the four topic branches are deleted.
- **Verification on merged develop, fresh `experiments_test`:** backend `models views api test_icp_handling services regression data_migrations` → 1,369 passed / 0 failed; vitest 34 files → 235 passed; `tsc`/`eslint` only the #106 baseline; `alembic heads` → `c4d8f1a2b6e7` alone.
- **Files changed (this entry's chore branch):** one unused import (`database/lineage_utils.py`) and seven over-120-char lines in the new files, all lint-only.
- **Issue #118:** left OPEN — PR4 (`chore/drop-legacy-note-columns`) is still to come, gated on the review queue emptying. Status comment posted on the issue.
- **Deploy runbook, in order:** (1) `alembic current` on the lab PC → expect `00063a5dd6a8` or earlier; (2) promote develop → main, nightly `update.ps1` runs `alembic upgrade head` (Ti + notes schema + notes views); (3) `fix_nan_text_fields_019.py --apply` (12 `'nan'` brine rows still on production); (4) `reclassify_notes_020.py` dry run, compare to `docs/issues/reclassify-notes-020-dryrun-2026-09-08.md`, then `--apply` — descriptions read NULL everywhere until this runs; (5) Power BI: rebind `v_dim_timepoints.modification_note`, drop `v_results_scalar.sampling_description`, add `v_notes`.
- **Tests added:** no. **Decision logged:** yes (`docs/working/decisions.md`, 2026-09-23).
...
- **Files changed:**
  ...
  - Tests: `tests/api/test_notes_bulk.py` (15), `tests/api/test_notes_review.py` (+7), `frontend/src/pages/__tests__/NotesReview.test.tsx` (10), `frontend/src/layouts/__tests__/AppLayout.badge.test.tsx` (2)
...
- **Fix rounds:** Task 4 (bulk/review API) had one round — `test_review_queue_path_is_not_captured_as_an_experiment_id` was hardened to keep its key-set assertion valid once `distinct_texts` was added. Task 6 (nav route/badge) had one round — the badge hide-at-zero test was made to wait for the query. Task 7's (page UI) review comments landed here rather than as a separate round: chip labels now distinguish loaded-vs-total counts when the loaded page is a truncated subset of the filter, the chip test pins union (not replace) semantics by pre-selecting a row the chip does not match and clicking it twice, and the header no longer shows "0 open" beside a load error.
- **Verification:** backend `tests/models tests/views tests/api tests/test_icp_handling.py tests/services tests/regression tests/data_migrations`, one process, `experiments_test` not reset (no model change on this branch) — 1,391 passed / 0 failed (baseline on develop 1,369, +22). Frontend `npx vitest run` — 247 passed / 0 failed (baseline 235, +12: 2 badge + 10 page; re-run by the controller after the final-review fix wave). `npx eslint src --ext .ts,.tsx` — 5 pre-existing problems, all outside files this branch touched (confirmed by re-running eslint against only the touched files: no output). `npx tsc --noEmit -p tsconfig.json` — only the 3 pre-existing `ResultsTab.columns.test.tsx` errors (`null` passed as `ScalarResult`), unrelated to this branch.
- **Final whole-branch review (fable, db85cf4..2a73ace):** "With fixes" — (1) the bulk mutations' `onError` did not invalidate `['notes-review']` or close the confirm, so a stale-id 404 under concurrent editing was a retry loop; (2) no test pinned the post-success refetch that keeps the nav badge honest. Both fixed in `f35fc5e` with a post-error refetch test and a `USER_MANUAL.md` Main Pages row. Left deferred: selection is wiped on any filter change (consequence of the approved "selection = loaded set" call; `placeholderData: keepPreviousData` would soften it), the `created_at` order test proves only the id tiebreak, the badge query runs the histogram GROUP BY (trivial at 1,288 rows), and single-note `delete_note` snapshots only `note_text` vs six fields in the bulk path — backport later. Correction: `note_text` is `Column(Text)`, nullable; zero NULL rows in practice, so the histogram's `None` guard is reachable but idle.
