...
- **Scope notes — Decision Points 1, 2, 6–10 outcomes:**
  ...
  7. A letterless `-t` vial (`SERUM_001-t7`) classifies as a **parent-like row** (base = stem, parent NULL) — kept as minimal-change default; verified harmless since `update_orphaned_derivations` matches parents by exact bare/`-0`/`-1` spelling only, so a `-t` vial can never steal orphans.
