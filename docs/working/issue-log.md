...
- **Files changed:**
  ...
  - Tests: `tests/api/test_notes_bulk.py` (15), `tests/api/test_notes_review.py` (+7), `frontend/src/pages/__tests__/NotesReview.test.tsx` (9), `frontend/src/layouts/__tests__/AppLayout.badge.test.tsx` (2)
...
- **Verification:** backend `tests/models tests/views tests/api tests/test_icp_handling.py tests/services tests/regression tests/data_migrations`, one process, `experiments_test` not reset (no model change on this branch) — 1,391 passed / 0 failed (baseline on develop 1,369, +22). Frontend `npx vitest run` — 245 passed / 0 failed (baseline 235, +10: 2 badge + 8 page). `npx eslint src --ext .ts,.tsx` — 5 pre-existing problems, all outside files this branch touched (confirmed by re-running eslint against only the touched files: no output). `npx tsc --noEmit -p tsconfig.json` — only the 3 pre-existing `ResultsTab.columns.test.tsx` errors (`null` passed as `ScalarResult`), unrelated to this branch.
