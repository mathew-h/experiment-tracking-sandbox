...
- **Verification:** `npx vitest run` → 244 passed in 35 files; `npx eslint src --ext .ts,.tsx` → 5 problems (#106 baseline); `npx tsc --noEmit` → 3 errors in `ResultsTab.columns.test.tsx` (baseline). Backend untouched — no pytest run needed for this branch. Chrome DevTools check of the wizard, Notes tab and dashboard card: see the PR body.
- **Tests added:** yes — 9. **Docs updated:** yes. **Decision logged:** no (decisions 7 and 8 were recorded in the phase-2 spec on 2026-09-24).
- **Final-review fixes (whole-branch review, opus, 2026-09-24):** retype select shows the chosen type from mutation state while pending and stays disabled until the refetch lands (`onSuccess` returns the invalidations); `['experiments']` is invalidated too so the list's Description column refreshes; `Step4Review.tsx` label "Condition Note" → "Description", guarded on trimmed text. Tests +4 (TypedNotes 2, new `Step4Review.test.tsx` 2). Verification after fixes: `npx vitest run` → 248 passed in 36 files; eslint 5 (baseline); tsc 3 (baseline). Accepted gap: one shared mutation tracks only the latest retype, so a second retype on another row re-enables the first early (data stays correct).
