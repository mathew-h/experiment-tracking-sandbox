...
**`GET /api/conditions/by-experiment/{experiment_id}` resolves through `experiment_fk`, never the stored `experiment_id` string** (issue #109 follow-up — that string is denormalized; both rename paths now sync it via `backend/services/denormalized_ids.py`, but 187 of 1013 rows stay stale until `dedupe_conditions_and_backfill_ids_018.py` runs, and `experiment_fk` is authoritative regardless). Lookup is `Experiment.experiment_id == path param` joined to `ExperimentalConditions.experiment_fk`, lowest id wins if more than one row exists. **404 means "this experiment has no conditions row"** — an unknown `experiment_id` also 404s with the same message (`"Conditions not found for this experiment"`); the response does not distinguish the two cases.

**`POST /api/conditions` returns 409** when a conditions row already exists for the submitted `experiment_fk` — `ExperimentalConditions` is 1:1 with `Experiment`, enforced by `UNIQUE (experiment_fk)` (`uq_conditions_experiment_fk`). `detail` is `"Conditions already exist for this experiment (id=<existing id>). Reload the page and edit them instead of adding new details."` Before this check, a stale-string 404 from the `by-experiment` lookup above made the detail page offer "Add Details" for an experiment that already had conditions, inserting a duplicate row with no way to detect it at request time. See `docs/issues/issue-duplicate-conditions-rows-and-stale-experiment-id-strings.md`.
