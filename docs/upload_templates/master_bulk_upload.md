...
### Optional Columns
Observation Note, Sample Collection Date, NMR Run Date, ICP Run Date, GC Run Date, XRD Run Date, NH4 (mM), H2 (ppm), Gas Volume (mL), Gas Pressure (psi), Sample pH, Sample Conductivity (mS/cm), Sampled Solution Volume (mL), Modification Note, Overwrite.

- **Free-text columns and typed notes (Dashboard template v4, issue #118).** v4 renamed `Description` → `Observation Note` and `Modification` → `Modification Note`. Both spellings are accepted for one deprecation window (`_HEADER_ALIASES`; the v3 names remain the parser's internal names), and a v3 and a v4 workbook of the same data produce identical result rows and identical note rows. Each cell is written twice during the transition: to the legacy `experimental_results.description` / `brine_modification_description` columns Power BI still reads, and as an `experiment_notes` row scoped to the result — Observation Note → `note_type='observation'`, Modification Note → `note_type='modification'`, `created_by='master_bulk_upload'`. One note slot per result and type: re-uploading with edited text updates the note in place rather than appending. A blank cell writes **no** note (nothing is required); the legacy column keeps its generated `Master upload — day N` fallback until PR3 removes it from the reporting view.
