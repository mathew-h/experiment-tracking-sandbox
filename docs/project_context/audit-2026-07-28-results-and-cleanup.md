...
**Resolved 2026-07-29, but re-confirm before the trigger's scope is fixed.** This
question was originally framed around `_OCCUPANCY_TYPES = {"hpht", "core flood"}`
at `experiment_status.py:17`. Issue #97 deleted that set along with the rest of
`experiment_status.py`'s local eligibility logic; the equivalent lookup now lives
as `_SERIES_BY_TYPE` in `database/reactor_slot.py`, which is likewise missing an
`"autoclave"` key by the same deliberate choice described below.

The underlying question stands regardless of which file holds the set: if
autoclave runs genuinely occupy HPHT vessels 1–7, then **Autoclave is an
...
**Answered "no" for #97 (2026-07-29):** encoded in `_SERIES_BY_TYPE` in
`database/reactor_slot.py`, whose comment records the reasoning — the audit found
`AUTO_JW_022`–`024` carrying historical HPHT vessel numbers, all COMPLETED and
therefore inert, so Autoclave was left out of the occupancy set for #97's scope.
Moot today (every `AUTO`/`Autoclave` row with a reactor number is COMPLETED), but it
determines whether the trigger scopes to two series or three, so this must be
re-confirmed with the team before `issue-reactor-occupancy-uniqueness-trigger.md`
fixes the trigger's scope. If the answer changes to "yes," add `"autoclave": "R"` to
`_SERIES_BY_TYPE`, to the dashboard's `in_(...)` filters, and to the trigger's scope
— nothing else in `database/reactor_slot.py` needs to change.
