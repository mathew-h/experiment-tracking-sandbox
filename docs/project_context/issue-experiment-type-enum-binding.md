...
- `database/reactor_slot.py::is_occupancy_type` (formerly `_is_eligible_for_occupancy`) *does* match it (it normalizes), so it participates in demotion. **Correction, 2026-07-30 (post-#97):** the "invisible in the UI" half of this no longer holds — the dashboard now keys occupancy and the grid on `reactor_slot`, which is derived via the same normalizing `is_occupancy_type`/`normalize_experiment_type` path, so a near-miss `experiment_type` like `"Core flood"` is no longer excluded from the reactor cards the way the old bare `experiment_type.in_([...])` filter excluded it. The row now participates in demotion *and* is visible in the UI.
