...
**`GET /api/conditions/by-experiment/{experiment_id}` resolves through `experiment_fk`, never the stored `experiment_id` string** (issue #109 follow-up — that string is denormalized and not kept in sync by rename paths; 187 of 1013 rows were stale as of 2026-08-05). Lookup is `Experiment.experiment_id == path param` joined to `ExperimentalConditions.experiment_fk`, lowest id wins if more than one row exists. **404 means "this experiment has no conditions row"** — an unknown `experiment_id` also 404s with the same message (`"Conditions not found for this experiment"`); the response does not distinguish the two cases.
