...
### Parameters
- `default_unit` (optional `str`, default `None`): When provided, unknown analyte headers are auto-created in the `Analyte` table using this unit. When `None`, unknown analyte headers are silently skipped.
- `overwrite` (`bool`, default `False`): Controls write behavior for existing records. See **Overwrite Behavior** below.

### Overwrite Behavior
Both `ElementalCompositionService` and `ActlabsRockTitrationService` share the same write contract:

| `overwrite` | Record exists? | Action |
|-------------|---------------|--------|
| `False` (default) | No | INSERT new record |
| `False` (default) | Yes | SKIP — existing value is preserved |
| `True` | No | INSERT new record |
| `True` | Yes | UPDATE to new value |

**Null/blank cells always mean "no data provided"** and are skipped regardless of the `overwrite` flag. They will never clear an existing value in the database.

Use `overwrite=False` (the default) for first-time uploads and partial re-uploads where you want to preserve previously entered values. Use `overwrite=True` when lab results have been revised after re-testing and you want to replace the stored values.
...
- **Data Flow:** Writes to the `ElementalAnalysis` table for each `(sample_id, analyte_id)` pair using the shared `_write_elemental_record` helper. Behavior depends on the `overwrite` flag (see above).
...
### Parameters
- `overwrite` (`bool`, default `False`): Controls write behavior for existing `ElementalAnalysis` records. See the **Overwrite Behavior** section under `ElementalCompositionService` above — both services share the same contract.
...
- **Result Write:** For every valid numeric cell, calls the shared `_write_elemental_record` helper for the corresponding `(sample_id, analyte_id)`. Non-detect cells (`nd`, `na`, `n/a`) and blank cells are skipped and never clear existing values.
- **Output:** Returns `(results_created, results_updated, skipped_rows, errors)`. Also provides a `diagnose` method to preview file structure and data quality before committing to the database.