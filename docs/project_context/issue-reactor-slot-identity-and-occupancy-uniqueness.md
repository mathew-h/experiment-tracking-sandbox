...
> `issue-reactor-occupancy-uniqueness-trigger.md` ([#112](https://github.com/mathew-h/experiment-tracking-sandbox/issues/112)), blocked on the cleanup in
...
>
> **Stale-reference note:** every `_is_eligible_for_occupancy` / `_normalize_type` /
> `_OCCUPANCY_TYPES` mention below (Background, Defect 1, Defect 3) describes the
> **pre-#97 code and no longer exists** — Task 4 of the branch deleted all three.
> The eligibility gate is now `derive_reactor_slot(reactor_number, experiment_type)
> is None` in `database/reactor_slot.py`, a single function with no local type set
> to keep in sync. Left the historical narrative below as written for context.
