...
| `backend/services/notes.py` | **Single definition** of legacy → typed mapping: `add_note`, `sync_result_note`. |
...
1. **`initial_note` is always the `description` note** (Mat, 2026-09-08: note[0] has always been the experiment description). The parser writes it only for a brand-new experiment or after an `overwrite=TRUE` row has cleared the old notes, so it is the oldest note by construction and the one-description index cannot fire.
...
- [ ] Write tests in `tests/services/bulk_uploads/test_typed_notes_dual_write.py` (section "helper"): sync creates, updates in place (same id), deletes on blank, ignores another created_by's note of the same type.
...
- [ ] Tests: blank `initial_note` + overwrite → seeded note survives, no `"nan"` row; new experiment with `initial_note` → one `description` note; existing experiment (no overwrite) with `initial_note` → row skipped, notes untouched; overwrite with text → old notes gone, one `ModificationsLog` row with the deleted texts, new `description` note.
- [ ] Implement: `initial_note = None if pd.isna(raw) else str(raw).strip() or None`; move the clearing block under `if initial_note:` and snapshot texts to `ModificationsLog(modification_type="delete", modified_table="experiment_notes", old_values={"note_texts": [...]})`; replace the `ExperimentNotes(...)` construction with `add_note(db, experiment, initial_note, note_type=NoteType.description, created_by="new_experiments")`.
