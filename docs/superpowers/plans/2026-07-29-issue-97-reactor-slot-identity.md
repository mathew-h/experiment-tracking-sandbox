...
- **Blocked on:** the Part A status changes (via the app UI) and Part B data migration `018` specified in `docs/issues/audit-2026-07-28-results-and-cleanup.md`. Neither has been done: `database/data_migrations/018*` does not exist, and as of 2026-07-30 (measured against the stored `reactor_slot` column) the dev DB has **4** double-booked slots — `CF01`×6, `CF03`×5, `R01`×6, `R06`×2 — plus 13 rows with `reactor_number = 0`. Prod had 2 slots and 11 zeros on 2026-07-28. **`R00` is not among the 4**, despite the audit listing it: those rows carry `reactor_number = 0`, so their `reactor_slot` is NULL and they form no slot — this branch already made that class inert. Keep the two prerequisites separate: the 4 collisions block the trigger, the 13 zero-rows block only the `CHECK`. **Both verification queries at the bottom of the audit file must return zero rows before this migration is written.**
